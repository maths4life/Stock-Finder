...
export function PageHeader({ eyebrow, title, description, action, className }: Props) {
  return (
    <header
      className={
        "animate-fade-up flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 " +
        (className ?? "")
      }
    >
      <div>
        ...
        {description && (
          <p className="mt-4 text-base text-ink-muted max-w-xl leading-relaxed text-pretty">
            {description}
          </p>
        )}
      </div>

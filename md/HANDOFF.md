...
- **Universe = 498 companies** (NIFTY 50 + Next 50 + an approximate Midcap150/Smallcap250 split by market-cap rank), as of Milestone 7 — `data/universe_nifty500.csv`, loaded by `backend/backend/ingest/universe.py`'s `load_universe()`. Previously ~100 (Milestone 5), before that 8 hardcoded companies. New `UNIVERSE_SIZE` env var can trim this for fast local iteration. Full NSE (~1500–2000) remains a future milestone; scaling to it should only require replacing/extending this one CSV, per `DECISIONS.md` ADR-015 — proven twice now (100→498 needed zero ingest-script code changes, same as 8→100 did).
...
**Milestone 9 — Weekly Production Refresh.** Changed the scheduled ingestion cadence from daily-weekdays to once-weekly, reordered the pipeline so `compute_scores` runs strictly last (after fundamentals and financial statements are refreshed, not just prices/technicals), and closed a real correctness gap found while doing this: none of the three network-calling ingest scripts (`fetch_prices.py`, `fetch_fundamentals.py`, `fetch_financial_statements.py`) previously failed their GitHub Actions step on a mostly-or-entirely-broken run — a total Yahoo outage would have printed "Failed: 498" and still exited 0, letting `compute_technicals`/`compute_scores` silently run against stale data while the workflow reported success. Fixed with a small, additive `assert_healthy()` helper in `ingest/resilience.py` (doesn't touch the existing `retry()`/`ConcurrentRunner` behavior at all — individual flaky-symbol failures still don't stop a run), called from each of the three scripts' `main()`. Also added a `workflow_dispatch`-only, opt-in step for `weekly_news_refresh.py` — **deliberately not added to the automatic weekly schedule**, since `TECHNICAL_DEBT.md` TD-008 (the news pipeline has never been verified against a live RSS feed) is still open and the project's own stated policy is to schedule it automatically only once that's closed; see §14 below for the full rationale and exact schedule/order. No routes, schemas, or frontend code were touched — this was a pure ingestion/ops milestone. See `CHANGELOG.md`'s Milestone 9 entry for the full file list.

**Verification performed this session:** a real local PostgreSQL 16 instance was stood up from a clean `schema.sql` load; the full pipeline (`fetch_prices` → `compute_technicals` → `fetch_fundamentals` → `fetch_financial_statements` → `compute_scores`, plus `weekly_market_intelligence.refresh_weekly_intelligence`) was run twice end-to-end against it with `yfinance`/`feedparser` monkeypatched to return realistic synthetic payloads (no live network access to Yahoo/NSE/RSS in this sandbox either — same limitation as Milestones 5–7), confirming every stage's upserts are idempotent (zero row-count growth on the second run) and that `journal_entries`/`pipeline_items` are untouched. `assert_healthy()`'s fail-loud path was separately verified by simulating an 83% failure rate and confirming `fetch_prices.main()` raised (non-zero exit) instead of reporting success. The FastAPI app was imported and its OpenAPI spec generated (all pre-existing routes present, none changed). The frontend was `npm install`'d, `npx tsc --noEmit`'d (unchanged pre-existing errors only — the same TD-022-class stale-mock-fixture/dead-tree errors documented before this milestone, not new ones), and `npm run build`'d, all successfully. The new `.github/workflows/ingest.yml` was validated as parseable YAML. See §14 for what remains unverified (the live network round-trip at the new cadence — same root cause as TD-027).

## 3a. Previous milestone

**Milestone 7 — Financial Statements, Shareholding Pattern, Universe Expansion to 498.** Three founder-specified modules in one session, each with its own detailed report: real Quarterly/Annual Financial Statement comparisons (`MODULE_7B_FINANCIAL_STATEMENTS_REPORT.md` — these tables existed and were correctly built, but always rendered N/A because no real multi-period data source existed; new `financial_statements` table + `ingest/fetch_financial_statements.py` fixes this), a full 7-category Shareholding Pattern breakdown with NSE-first/yfinance-fallback sourcing (`MODULE_7C_SHAREHOLDING_REPORT.md`), and expanding the universe from ~100 to 498 companies plus hardening the two network-calling ingest scripts with retry/concurrency (`MODULE_8_UNIVERSE_EXPANSION_REPORT.md`). See `CHANGELOG.md`'s Milestone 7 entry for the full file-by-file index. A real local PostgreSQL instance was stood up specifically to verify this milestone end-to-end (schema, services, and the full FastAPI app via `TestClient`) rather than relying on syntax checks alone — a first for this project's verification process. **Two honest gaps carried forward, both logged as TD-027/TD-028:** none of the new/changed network calls (yfinance statement endpoints, NSE's shareholding API) were exercised against the live internet — this session's environment couldn't reach either host — and the universe's Midcap150/Smallcap250 tagging is a market-cap-rank approximation, not a verified read of NSE's official constituent lists.

## 3b. Milestone before that
...
8. **TD-027** (🟠) — Milestone 7's four new/changed network-calling ingest scripts (financial statements, shareholding NSE+yfinance, prices, fundamentals) have never run against live yfinance/NSE — same network-restricted-build-environment root cause as TD-008.
9. **TD-028** (🟡) — Milestone 7's 498-company universe's Midcap150/Smallcap250 tags are a market-cap-rank approximation, not verified against NSE's official constituent lists.

Resolved as of Milestone 9: TD-009's manual-trigger half (a `workflow_dispatch` opt-in for the news refresh now exists — its automatic-schedule half stays blocked on TD-008, still open). Newly logged: TD-029 (weekly-cadence live-network verification gap), TD-030 (found-but-not-fixed `.env`/`.gitignore` secret-exposure risk, out of this milestone's scope to fix unilaterally).

Resolved as of Milestone 7: TD-001's "real fix" half (frozen fundamentals snapshot) — a genuine multi-period `financial_statements` table now exists, sourced live from yfinance. Newly logged: TD-027, TD-028.
...
7. **Close out Milestone 7's verification gaps (TD-027)** — run `fetch_financial_statements.py`, `fetch_shareholding.py`, `fetch_prices.py`, `fetch_fundamentals.py` each with `--limit 10` against a real internet connection and confirm the logged output looks sane before relying on any of Milestone 7's new data. Should happen before the daily cron is trusted at 498-company scale.
8. **A visible "as of" staleness indicator** — TD-001's one remaining open half (the data-freshness *fix* is now substantially done as of Milestone 7; the UI never told the founder how fresh a number was, and still doesn't).
...
## 14. Weekly production refresh (Milestone 9)

**Schedule:** `cron: "30 20 * * 5"` in `.github/workflows/ingest.yml` — Fridays 20:30 UTC, which is Saturdays 02:00 IST, i.e. Saturday early morning IST, after Friday's NSE close (15:30 IST). Day-of-week is pinned to Friday only, so this fires exactly once a week. `workflow_dispatch` still exists for an on-demand full manual run at any time.

**Execution order (dependency-ordered, unchanged reasoning from the daily job, just less frequent):**
1. `fetch_prices` — incremental OHLCV upsert into `prices_daily`.
2. `compute_technicals` — recomputes `technical_snapshot` from the freshly-updated prices.
3. `fetch_fundamentals` — refreshes `financials_quarterly`'s `'latest'` row, the yfinance-approximation `shareholding_pattern` rows, and `companies.sector`/`.industry` metadata enrichment.
4. `fetch_financial_statements` — refreshes the real multi-period `financial_statements` table (quarterly + annual).
5. *(manual opt-in only, not on the automatic schedule)* `weekly_news_refresh` — see below.
6. `compute_scores` — always last, so scores are computed from this week's fundamentals/financial-statements/technicals, not last week's.

**What's refreshed weekly vs. what stays user-generated:** `companies`, `prices_daily`, `technical_snapshot`, `financials_quarterly`, `shareholding_pattern`, `financial_statements`, and `scores` are all refreshed by the steps above. `journal_entries`, `journal_reviews`, and `pipeline_items` are never touched by this workflow — they're written only through their own `POST`/`PUT`/`PATCH`/`DELETE` routes, on user action, exactly as before this milestone.

**Idempotency:** every write in every step above is an `insert ... on conflict (...) do update` upsert (verified directly against `db/schema.sql`'s primary keys and each ingest script's SQL, and confirmed empirically this session — running the full pipeline twice against a real local Postgres produced zero row-count growth on the second run). `fetch_prices` is additionally incremental (only requests trading days after each symbol's latest stored date), so a mid-week manual re-run via `workflow_dispatch` is cheap, not just safe.

**Manual trigger:** GitHub → Actions tab → "Weekly data refresh" → Run workflow. Leave the `run_news_refresh` checkbox unchecked for a normal full-pipeline run (prices → technicals → fundamentals → financial statements → scores); check it only when deliberately testing the news pipeline against a real feed (see the note below).

**Failure handling:** each of the three network-calling steps (`fetch_prices`, `fetch_fundamentals`, `fetch_financial_statements`) keeps its existing per-symbol retry-with-backoff and continue-past-individual-failures behavior (`ingest/resilience.py`'s `retry()`/`ConcurrentRunner`, unchanged) — a handful of flaky symbols doesn't stop the run. New this milestone: each step now also calls `assert_healthy()`, which raises (non-zero exit, failing that GitHub Actions step) if the run's failure rate exceeds 50% or if literally everything failed. Because a GitHub Actions job stops at the first failed step by default, a broken `fetch_prices` run can no longer silently let `compute_technicals`/`compute_scores` compute from stale/missing data while the workflow still shows green. `compute_scores` itself has no network calls and will raise naturally on any DB error.

**News/weekly-intelligence refresh — deliberately not automated:** `TECHNICAL_DEBT.md` TD-008 documents that `weekly_news_refresh.py` has never been run against a live RSS feed in any session so far (no outbound network access to Google News/Moneycontrol/Economic Times/Business Standard/Yahoo's RSS endpoint in this or prior sandboxes), and the project's own stated policy (`DATA_STRATEGY.md` §5) is to add it to the automatic schedule only after that verification happens. Rather than silently enable an unverified network-calling step on a schedule, this milestone added it as a `workflow_dispatch`-only opt-in (a `run_news_refresh` boolean input, default off, never triggered by the `schedule:` cron) — this makes it easy to run manually for the real-feed test TD-008 asks for, without changing its production-readiness status. **Do not remove the `if: github.event_name == 'workflow_dispatch'` guard to "just enable it" without actually doing that live-feed verification first.**

**Known limitations carried into this milestone (see `TECHNICAL_DEBT.md` TD-029):** the weekly-cadence change itself, and the new `assert_healthy()` check, have been verified against a real local Postgres with synthetic (monkeypatched) Yahoo/RSS data — not against live `yfinance`/NSE at the new cadence, same network-restricted-sandbox limitation as TD-027 one milestone earlier. The 50% failure-rate threshold in `assert_healthy()` is a reasonable default, not empirically tuned against real-world weekly failure rates — worth watching over the first few real scheduled runs.

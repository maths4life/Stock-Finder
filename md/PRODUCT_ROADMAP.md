...
- ~~Load-test the daily cron (`fetch_prices` → `compute_technicals` → `compute_scores`) against the larger universe — `yfinance` rate limits and GitHub Actions job time limits are real constraints at this scale and should be verified, not assumed.~~ **Not done in Milestone 5** — estimated but not measured against a live run; see `TECHNICAL_DEBT.md` TD-023/TD-024. Worth doing before scaling further. **Note (Milestone 9):** the cron is no longer daily — it's now weekly (`fetch_prices` → `compute_technicals` → `fetch_fundamentals` → `fetch_financial_statements` → `compute_scores`, see `HANDOFF.md` §14) — but this item's underlying ask (verify real yfinance/GitHub-Actions behavior at 498-company scale against a live connection, not just estimate it) is still open, now tracked as `TECHNICAL_DEBT.md` TD-027/TD-029.

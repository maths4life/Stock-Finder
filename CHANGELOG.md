...
- `backend/backend/analysis/scoring_engine.py` — the new single source of truth for `fundamentalScore`/`technicalScore`/`overallScore`. Pure functions (`score_fundamentals`, `score_technicals`, `compute_scores`), no DB access, no I/O — takes a flat dict of already-fetched metric values, returns scores plus a full per-metric breakdown (`value`/`score`/`maxScore`/`passed`/`reason`). Called by both `services/company_service.py` (live) and `ingest/compute_scores.py` (batch), so the two can never disagree. 11 fundamental metrics / 100 pts, 6 technical metrics / 100 pts, 60/40 fundamental/technical weighting (previously an unstated 50/50 — see `SCORING_ENGINE.md` §0.1). Missing data is excluded from a metric's contribution (score=0, maxScore=0) instead of defaulted to a neutral guess, a direct fix for `SCORING_ENGINE.md` §3's "missing data silently defaulted" problem. New real metrics: sector-relative P/E (peer average from `companies.sector` + `financials_quarterly.pe`, real data, min. 2 peers, absolute-band fallback otherwise), Promoter Holding (wires in previously-unused `shareholding_pattern` data), and a Golden/Death Cross metric (see below). No indicator not already computable from existing tables was invented.

### Backend — files modified
- `backend/backend/ingest/indicators.py` — added `detect_death_cross()`, the bearish mirror of the existing `detect_golden_cross()` (MA50 crossing below, not above, MA200 within the lookback window). Same logic shape, reused rather than duplicated.
- `backend/backend/ingest/compute_technicals.py` — computes and persists the new `death_cross` column alongside the existing `golden_cross`.
- `backend/backend/db/schema.sql` — added `technical_snapshot.death_cross boolean`, via the file's existing `alter table add column if not exists` migration-guard convention (additive, non-destructive on an existing database).
- `backend/backend/ingest/compute_scores.py` — rewritten to be a thin data-plumbing layer over `analysis/scoring_engine.py`: fetches the wider field set the engine needs (now includes `pb`, `peg`, `dividend_yield_pct`, `current_ratio`, promoter holding from `shareholding_pattern`, `high_52w`/`low_52w`, latest volume, `death_cross` — previously only ROE/ROCE/profit growth/P/E and a handful of technical flags were fetched), computes a batched sector-average-P/E map (one query for the whole universe), calls the shared engine, and persists `fundamental_score`/`technical_score`/`overall_score`/`verdict`/`rationale` to the `scores` table exactly as before (so Discover/Screener's SQL-level `order by s.overall_score` keeps working unchanged).
- `backend/backend/services/company_service.py` — `BASE_QUERY` widened to also select `t.death_cross`, `t.high_52w`, `t.low_52w` (same already-joined `technical_snapshot` table, no new join). Added `_fetch_sector_avg_pe()`, a batched one-query-per-request-cycle lookup mirroring the one in `compute_scores.py`. `_company_fields()` now computes `fundamentalScore`/`technicalScore`/`overallScore`/`verdict` **live**, via `analysis.scoring_engine.compute_scores()`, from the fields it already fetched — no longer trusts the (potentially stale, only refreshed by the daily batch job) `scores` table columns for these three numbers. This is what guarantees the score shown on a company card and the "Why this score?" breakdown on that same company's detail page can never disagree — see `SCORING_ENGINE.md` §5. `weighting` and `scoreBreakdown` are attached to the field dict for every row; only the `Company` (detail) Pydantic model actually declares/serializes them, so `CompanyListItem` (the list endpoint) is unaffected in payload size.
- `backend/backend/schemas/company.py` — added `ScoreMetric`, `ScoreBreakdown`, `ScoreWeighting` models (mirroring `analysis/scoring_engine.py`'s output shape exactly); added `weighting: ScoreWeighting` and `scoreBreakdown: ScoreBreakdown` to the `Company` (detail-only) model; added `deathCross: bool` to `CompanyBase` (so it's on both list and detail responses, alongside the pre-existing `goldenCross`).

### Frontend — files created
- `frontend/src/features/company/components/ScoreBreakdown.tsx` — the "▼ Why this score?" panel. A `Collapsible` (collapsed by default, per the brief's "keep the page clean when collapsed") that, when expanded, shows: the weighting formula stated with the company's real numbers (`82 = 60% Fundamental (86) + 40% Technical (78)` style, built from `c.weighting`/`c.overallScore`/`c.fundamentalScore`/`c.technicalScore`, no hardcoded copy); a Fundamental Breakdown and Technical Breakdown section, each metric showing its name, real value, points earned/available, a green check / red cross pass-fail icon (amber triangle for "no data"), a plain-English reason, and a green/amber/red progress bar sized to points-earned-as-percentage. Renders exclusively from `company.scoreBreakdown`/`company.weighting` — zero calculation happens client-side, satisfying the brief's "no calculations should exist in the frontend."

### Frontend — files modified
- `frontend/src/shared/api/types.ts` — added `ScoreMetric`, `ScoreBreakdown`, `ScoreWeighting` types (mirroring the new backend schemas); added `deathCross: boolean` and optional `weighting?`/`scoreBreakdown?` fields to `Company` (optional because the lighter list endpoint doesn't send them — the single merged frontend `Company` type already covers both `CompanyListItem` and `Company` API shapes, as it did before this change).
- `frontend/src/routes/research.$symbol.tsx` — renders `<ScoreBreakdownPanel company={c} />` directly under the existing Overall/Fundamental/Technical score row.
- `frontend/src/features/company/api/mock/companies.data.ts` — added `deathCross: false` to all 8 mock records (this file is dead/unused at runtime — nothing imports it — but is still type-checked by `tsc`, so the new required `Company` field needed a value here to keep the build clean).

### Verification performed
- `analysis/scoring_engine.py` unit-tested standalone (full-data case, empty-input case, partial-data case, death-cross path, no-sector-avg fallback): confirmed the fundamental/technical breakdown always sums to exactly the displayed `fundamentalScore`/`technicalScore`, confirmed `overallScore` always equals exactly `0.6×fundamental + 0.4×technical` (or falls back correctly when one side has zero data), confirmed missing data is excluded rather than scored. Two real bugs were caught and fixed by this testing before shipping: (1) `_score_pe` was scoring "no P/E data" the same as "P/E present but not meaningful" (both landed on `maxScore=10`, defeating the missing-data exclusion for that metric); (2) the Golden/Death Cross metric had no way to represent "no crossover data available" — both flags being `None` fell through to the same "no cross detected" neutral branch as both being genuinely `False`.
- `backend/backend/app.py` and every modified backend module imported successfully with real dependencies installed (`fastapi`, `sqlalchemy`, `psycopg2-binary`, `pandas`, etc.) — confirms no import-time errors, no undefined names, no signature mismatches introduced by threading the new engine through `company_service.py` and `compute_scores.py`.
- Frontend: `npx tsc --noEmit` run against the full project — zero new errors introduced by this change (confirmed by name-filtering the output for the new/modified files). The ~20 pre-existing errors in the output are all in unrelated dead-code trees (`lib/api/*`, unused mock files under `features/*/api/mock/`, an incomplete `AIResearchReport.tsx`/`WeeklyMarketIntelligence.tsx` pairing) predating this session — see `TECHNICAL_DEBT.md` TD-002. `npm run build` (`vite build`) completes successfully.
- Confirmed `services/discover_service.py` and `services/screener_service.py`'s direct SQL reads of `scores.overall_score`/`scores.technical_score` need no changes — the `scores` table's schema and write contract are unchanged, only what feeds its values changed.

### Not touched this session
- The v2 target architecture in `SCORING_ENGINE.md` §4 (sector-relative percentile scoring across all factors, multi-quarter earnings-quality trend, the calibration loop against `journal_reviews`) — explicitly out of scope per that document's own "do not build this until the universe has grown meaningfully and there is real journal history" guidance, and not part of this milestone's brief. FII/DII holding *trend* (as opposed to level) was evaluated and deliberately not scored — see `SCORING_ENGINE.md` §0.3 for why.

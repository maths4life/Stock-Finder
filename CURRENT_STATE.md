...
| `financials_quarterly`, `shareholding_pattern` | `ingest/seed_fundamentals.py` (one-time historical Kaggle import, `source = 'kaggle_seed'`), then kept live by `ingest/fetch_fundamentals.py`'s daily yfinance refresh (`source = 'yfinance'`/`'yfinance_approx'`) | `financials_quarterly`'s `'latest'` row and `shareholding_pattern`'s approximated categories refresh daily via the scheduled workflow. Real multi-quarter/NSE-sourced history for either table depends on the new Milestone 7 pipelines (`financial_statements`, and `shareholding_pattern`'s `source='nse'` path) — see the rows below and `MODULE_7C_SHAREHOLDING_REPORT.md`. |

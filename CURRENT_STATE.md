...
| **3. Company deep detail** | `services/fundamental_service.py`, `services/technical_service.py`, `services/financial_statements_service.py`, `services/shareholding_service.py` (new, Milestone 7) | `research.$symbol.tsx` | ✅ | Pros/cons, checklist, verdict genuinely computed from stored data. **Milestone 7:** Quarterly/Annual Financial Comparison tables were previously always N/A (the old comparison logic had no real multi-period data to read — see `MODULE_7B_FINANCIAL_STATEMENTS_REPORT.md`); now backed by a real `financial_statements` history table populated from yfinance's statement endpoints. Shareholding trend extended from a 4-category to a full 7-category breakdown with a latest/previous-quarter summary (`MODULE_7C_SHAREHOLDING_REPORT.md`). Both unverified against a live yfinance/NSE connection in this sandbox — see those reports' "Honest limitations." |
...
### Universe expanded from 8 to ~100 companies (Milestone 5), then to 498 (Milestone 7)
...
**Milestone 7 update:** default universe grew again, to 498 companies (`data/universe_nifty500.csv`, NIFTY 50 + Next 50 + an approximate Midcap150/Smallcap250 split by market-cap rank — see `MODULE_8_UNIVERSE_EXPANSION_REPORT.md` for the full provenance and honesty notes on that approximation). Same pattern held: zero code changes needed in any script that reads `UNIVERSE`/`load_universe()`. New `UNIVERSE_SIZE` env var added for fast local iteration without a separate CSV. `fetch_prices.py`/`fetch_fundamentals.py` (the two scripts that make external network calls) gained retry-with-backoff and bounded concurrency (`ingest/resilience.py`) for reliability at the larger scale — this session's local DB testing verified the logic; a live 498-company run has not been executed (same network-sandbox restriction as elsewhere in this milestone).
...
|---|---|---|
| `prices_daily`, `technical_snapshot`, `scores` | GitHub Actions cron, **weekly, Fridays 20:30 UTC / Saturdays 02:00 IST** (`.github/workflows/ingest.yml`) → `fetch_prices` → `compute_technicals` → `fetch_fundamentals` → `fetch_financial_statements` → `compute_scores` | Genuinely fresh, weekly, for the tracked 498-symbol universe (Milestone 9: changed from daily-weekdays to weekly, and reordered so `compute_scores` runs last, after fundamentals/financial-statements are refreshed too — see `HANDOFF.md` §3 for the exact rationale. `fetch_prices` remains incremental — only fetches trading days after each symbol's latest stored date, not a full 2y re-pull every run). Each network-calling stage now fails the workflow loudly (non-zero exit via `ingest/resilience.py`'s `assert_healthy()`) if most/all of that stage's attempted symbols failed, instead of silently reporting success on a broken run — see `HANDOFF.md`'s "failure handling" section. |
| `financials_quarterly`, `shareholding_pattern` | `ingest/seed_fundamentals.py` (one-time historical Kaggle import, `source = 'kaggle_seed'`), then kept live by `ingest/fetch_fundamentals.py`'s now-weekly yfinance refresh (`source = 'yfinance'`/`'yfinance_approx'`) | `financials_quarterly`'s `'latest'` row and `shareholding_pattern`'s approximated categories refresh weekly via the scheduled workflow (changed from daily in Milestone 9 — see above). Real multi-quarter/NSE-sourced history for either table depends on the Milestone 7 pipelines (`financial_statements`, and `shareholding_pattern`'s `source='nse'` path) — see the rows below and `MODULE_7C_SHAREHOLDING_REPORT.md`. |
| `news_articles`, `weekly_sector_intelligence` | `ingest/weekly_news_refresh.py`, present in `.github/workflows/ingest.yml` as a **manual `workflow_dispatch` opt-in step only** (Milestone 9) — deliberately **not** on the scheduled weekly trigger | Still not run automatically. TD-008 (never run against a live RSS feed) is the reason: the project's own stated policy (`DATA_STRATEGY.md` §5, `TECHNICAL_DEBT.md` TD-009) is to add this to the schedule only after it's been proven against real feeds — that verification still hasn't happened (this and prior sessions' sandboxes have no route to the RSS providers), so Milestone 9 added the opt-in manual trigger but left the automatic schedule alone rather than silently enabling an unverified step. See `HANDOFF.md` §"News pipeline" note. |
...
| `financial_statements` (new, Milestone 7) | `ingest/fetch_financial_statements.py`, scheduled in `.github/workflows/ingest.yml` alongside fundamentals, now weekly (Milestone 9) | Real multi-period history once ingestion runs (logic verified against real Postgres + synthetic yfinance-shaped data; a live yfinance run has not been executed in this sandbox — see `MODULE_7B_FINANCIAL_STATEMENTS_REPORT.md`). |
| `shareholding_pattern`'s NSE-sourced categories (new, Milestone 7.5) | `ingest/fetch_shareholding.py` → `ingest/nse_client.py`, delegated-to from the weekly `ingest/fetch_fundamentals.py` step | `source='nse'` rows would be genuinely fresh NSE disclosures; in practice this session's NSE client is unverified live (this sandbox cannot reach `nseindia.com`) and every row degrades safely to `source='yfinance_approx'` until that's confirmed working — see `MODULE_7C_SHAREHOLDING_REPORT.md`. |
...
- The weekly ingest cron for prices/technicals/fundamentals/financial-statements/scores (changed from daily-weekdays in Milestone 9), including its per-stage fail-loud safety check.
